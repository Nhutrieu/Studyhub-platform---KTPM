...
async function submitReply(postId, parentId) {
    ...
    // 🟢 FIX: LẤY TOÀN BỘ HTML LÀM NỘI DUNG
    ...
    if (mentionTag) {
        ...
        const mentionedName = mentionTag.textContent.trim();
        ...
        // 🟢 FIX: LOẠI BỎ TÊN TEXT TRÙNG VỚI TAG
        // Kiểm tra nếu text content có chứa tên mention
        const textContent = input.textContent || '';
        
        // Nếu text content có tên mention (có thể ở đầu hoặc giữa)
        if (textContent.includes(mentionedName)) {
            // Tạo regex để loại bỏ tên mention
            const nameRegex = new RegExp(mentionedName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g');
            
            // Lấy HTML, loại bỏ text node chứa tên mention
            ...
            // Loại bỏ tên mention từ text nodes
            textNodes.forEach(textNode => {
                if (textNode.textContent.includes(mentionedName)) {
                    textNode.textContent = textNode.textContent.replace(nameRegex, '').trim();
                    
                    // Nếu text node trống, xóa nó
            ...
            // Loại bỏ tag mention trùng nếu có
            const allTags = tempDiv.querySelectorAll('.user-mention');
            if (allTags.length > 1) {
                // Chỉ giữ lại tag đầu tiên
                const firstTag = allTags[0].outerHTML;
                // Xóa tất cả tags
                allTags.forEach(tag => tag.remove());
                // Thêm lại tag đầu tiên vào đầu
                tempDiv.innerHTML = firstTag + tempDiv.innerHTML;
                finalContent = tempDiv.innerHTML.trim();
            }
            
            console.log('🟡 Cleaned HTML (removed duplicate name):', finalContent);
        }
    } else {
        // Không có tag mention
        finalContent = input.textContent.trim();
    }
    
    // 🟢 LOẠI BỎ TAG TRÙNG LẶP CUỐI CÙNG
    const tempCheckDiv = document.createElement('div');
    tempCheckDiv.innerHTML = finalContent;
    const allMentionTags = tempCheckDiv.querySelectorAll('.user-mention');
    
    if (allMentionTags.length > 1) {
        console.log('🟡 Detected duplicate mention tags:', allMentionTags.length);
        
        // Chỉ giữ lại tag đầu tiên
        const firstTag = allMentionTags[0].outerHTML;
        
        // Xóa tất cả text nodes
        const textWalker = document.createTreeWalker(tempCheckDiv, NodeFilter.SHOW_TEXT, null, false);
        let textNode;
        let allText = '';
        ...
        // Loại bỏ tên mention từ text
        if (allMentionTags[0].textContent) {
            const name = allMentionTags[0].textContent.trim();
            allText = allText.replace(new RegExp(name, 'g'), '').trim();
        }
        
        // Tạo lại content
        finalContent = firstTag + (allText ? ' ' + allText : '');
        
        console.log('🟡 Fixed duplicate tags:', finalContent);
    }
...
function buildCommentImageURL(imagePath) {
    ...
    // Nếu bắt đầu bằng /uploads/comments/ (từ database)
...
function renderReplyItem(r) {
    ...
    <div class="comment-item reply-level" 
        ...
        <div class="comment-avatar" onclick="if(window.openUserProfile)window.openUserProfile('${username}')">
            ${avatarUrl ? `
        ...
        <div class="comment-body">
            <div class="comment-bubble">
                <div class="comment-name" onclick="if(window.openUserProfile)window.openUserProfile('${username}')">
                    ${nameWithBadge}
                </div>
...
function cleanCommentContent(html) {
    ...
    // 🟢 FIX: LOẠI BỎ TAG TRÙNG LẶP
    // Tìm tất cả các user-mention tags
    const mentionRegex = /<span class="user-mention"[^>]*data-user-id="(\d+)"[^>]*data-username="([^"]*)"[^>]*>([^<]+)<\/span>/g;
    const mentions = [];
    let match;
    ...
    // 🟢 NẾU CÓ NHIỀU HƠN 1 MENTION CÙNG USER, LOẠI BỎ TRÙNG
    let cleaned = html;
    ...
    if (mentions.length > 1) {
        ...
        mentions.forEach((mention, index) => {
            if (!groupedByUser[mention.userId]) {
                groupedByUser[mention.userId] = [];
            }
            groupedByUser[mention.userId].push({...mention, index});
        });
        
        // Giữ lại mention đầu tiên của mỗi user
        const mentionsToKeep = [];
        Object.values(groupedByUser).forEach(userMentions => {
            if (userMentions.length > 0) {
                mentionsToKeep.push(userMentions[0]);
            }
        });
        
        // Xây dựng lại HTML
        if (mentionsToKeep.length < mentions.length) {
            let result = '';
            let lastIndex = 0;
            
            mentionsToKeep.forEach(mention => {
                // Thêm phần trước mention
                result += cleaned.substring(lastIndex, mention.start);
                // Thêm mention (giữ lại)
                const mentionHtml = cleaned.substring(mention.start, mention.end);
                result += mentionHtml;
    ...
    // 🟢 XỬ LÝ "NGƯỜI DÙNG MỚI" TRONG TAG
    cleaned = cleaned.replace(
        /<span class="user-mention"[^>]*data-user-id="(\d+)"[^>]*data-username="([^"]*)"[^>]*>([^<]+)<\/span>/g,
        function(match, userId, username, fullName) {
            // 🟢 NẾU LÀ "NGƯỜI DÙNG MỚI", DÙNG USERNAME
            if (fullName.includes("Người dùng mới") || fullName.includes("New User")) {
                return `<span class="user-mention" data-user-id="${userId}" data-username="${username}">${username}</span>`;
            }
            // 🟢 GIỮ NGUYÊN NẾU BÌNH THƯỜNG
            return match;
        }
    );
    
    // 🟢 LOẠI BỎ TAG RỖNG HOẶC TRÙNG LẶP SAU KHI XỬ LÝ
    cleaned = cleaned.replace(/<span class="user-mention"[^>]*><\/span>/g, '');
    cleaned = cleaned.replace(/(<span class="user-mention"[^>]*>.*?<\/span>)\s*\1/g, '$1');
...
function editComment(commentId, event) {
    ...
    // 🟢 TẠO EDIT BOX ĐƠN GIẢN (giống comment box)
    const editBoxHTML = `
        <div class="comment-edit-box" id="comment-edit-${commentId}">
            <div class="comment-edit-input-wrapper">
                <textarea class="comment-edit-input" 
                          placeholder="Chỉnh sửa bình luận..."
                          rows="3">${plainText}</textarea>
                
                <div class="comment-edit-buttons">
                    <button class="comment-edit-add-image" 
                            onclick="document.getElementById('edit-image-input-${commentId}').click()">
                        <span class="material-icons">image</span>
                    </button>
                    <button class="comment-edit-send" onclick="saveCommentEditSimple(${commentId}, event)">
            ...
            <div class="comment-edit-preview-container" id="edit-preview-${commentId}">
                ${hasCurrentImage ? `
                    <div class="comment-edit-preview" id="current-image-${commentId}">
            ...
            <div class="comment-edit-controls-simple">
                <button class="cancel-edit-btn" onclick="cancelCommentEditSimple(${commentId}, event)">Hủy</button>
                <div class="edit-tips">Ảnh bị xóa sẽ không được lưu</div>
            </div>
        </div>
    `;
...
async function saveCommentEditSimple(commentId, event) {
    ...
    try {
        ...
        console.log('🟡 Image status for edit:', { 
            hasNewImage, 
            hasPreview,
        ...
        if (hasNewImage) {
            ...
            // ĐÃ XÓA ẢNH GỐC (không có preview)
        ...
        // 🟢 CẬP NHẬT GIAO DIỆN - CẢI THIỆN PHẦN NÀY
        commentTextElement.innerHTML = newContent.replace(/\n/g, '<br>');
        commentTextElement.style.display = 'block';
        
        // 🟢 QUAN TRỌNG: XỬ LÝ ẢNH SAU KHI CẬP NHẬT
        if (data.image_url || data.comment?.ImageURL) {
            // CÓ ẢNH MỚI HOẶC ẢNH ĐƯỢC GIỮ LẠI
            const imageUrl = data.image_url || buildCommentImageURL(data.comment.ImageURL);
            const imageHTML = `
                <div class="comment-image-container">
                    <div class="comment-image" onclick="openSimpleImagePreview('${imageUrl}')">
                        <img src="${imageUrl}" alt="Ảnh đính kèm" loading="lazy">
                    </div>
                </div>
            `;
            
            if (commentImageContainer) {
                // Thay thế container ảnh cũ
                commentImageContainer.outerHTML = imageHTML;
            } else {
                // Thêm container ảnh mới
                commentTextElement.insertAdjacentHTML('afterend', imageHTML);
            }
            console.log('✅ Image updated in UI:', imageUrl);
            
        } else if (data.remove_image || data.action === 'remove') {
            // ĐÃ XÓA ẢNH
            if (commentImageContainer) {
        ...
        // 🟢 THÔNG BÁO
        showToast('✅ Đã cập nhật bình luận và ảnh');
